...
export default function DynamicCarDetailsForm() {
  ...
  const variants = useMemo(() => selectedYear?.variants.map((v) => v.name) || [], [selectedYear]);

  const selectedVariant =
    selectedYear?.variants.find((v) => v.name === variant) ||
    (variants.length === 1 ? selectedYear?.variants[0] : null);

  const isMileageIncorrect = kilometers && parseInt(kilometers) !== selectedVariant.mileage;
        ...
        <div className="flex md:flex-row gap-y-4 flex-col justify-center md:items-end">
          <form onSubmit={handleSubmit} className="space-y-6 md:px-16 flex-1">
            <div className="space-y-2">
              <label className="block text-lg font-medium text-gray-700">Von welcher Marke ist dein Auto?</label>
              <CustomSelect
                value={brand}
            ...
            <div className="space-y-2">
              <label className="block text-lg font-medium text-gray-700">Welches Modell?</label>
              <CustomSelect
                value={model}
            ...
            <div className="space-y-2">
              <label className="block text-lg font-medium text-gray-700">In welchem Jahr wurde es zugelassen?</label>
              <CustomSelect
                value={year}
            ...
            {year && (
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="block text-lg font-medium text-gray-700">Welche Bauform hat es?</label>
                  {bodyStyles.length === 1 ? (
                    <input
                      type="text"
                      value={bodyStyles[0]}
                    ...
                    <SelectableButtons options={bodyStyles} selectedValue={bodyStyle} onSelect={setBodyStyle} />
                  )}
                </div>
                {(bodyStyle || bodyStyles.length === 1) && (
                  <div className="space-y-2">
                    <label className="block text-lg font-medium text-gray-700">Welchen Kraftstoff tankt es?</label>
                    <input
                      type="text"
                      value={fuelType}
                      className="w-full px-4 py-3 text-center bg-white border-2 text-[#EE202C] font-medium border-[#EE202C] rounded text-base focus:outline-none"
                ...
                {(bodyStyle || bodyStyles.length === 1) && (
                  <div className="space-y-2">
                    ...
                    {variants.length === 1 ? (
                      ...
                      <SelectableButtons options={variants} selectedValue={variant} onSelect={setVariant} />
                    )}
                  </div>
                )}
                ...
                {(bodyStyle || bodyStyles.length === 1) && selectedVariant && (
                  <div className="space-y-6">
                    <div className="space-y-2">
                      <label className="block text-lg font-medium text-gray-700">Wie viele PS hat es?</label>
                      <SelectableButtons options={selectedVariant.power} selectedValue={power} onSelect={setPower} />
                    </div>
                    {power && (
                      <div className="space-y-2">
                        <label className="block text-lg font-medium text-gray-700">Welches Getriebe hat es?</label>
                    ...
                    {power && (transmission || selectedVariant.transmission.length === 1) && (
                      <div className="space-y-2">
                        <label className="block text-lg font-medium text-gray-700">Wie viele Türen hat es?</label>
                        <input
                          type="text"
                    ...
                    {power && (transmission || selectedVariant.transmission.length === 1) && (
                      <div className="space-y-2">
                        <label className="block text-lg font-medium text-gray-700">Welcher Modelltyp ist es?</label>
                        <input
                          type="text"
                    ...
                    {power && (transmission || selectedVariant.transmission.length === 1) && (
                      <div className="space-y-2">
                        <label className="block text-lg font-medium text-gray-700">Wie ist der Kilometerstand?</label>
                        <div className="flex items-center relative">
                          ...
                          />
                          <span className="absolute top-3 right-3 text-lg">km</span>
                        </div>
                        {isMileageIncorrect && (
                          <div className="mt-2 p-3 bg-[#EE202C]/20 border border-[#EE202C] text-[#EE202C] rounded">
                            Basierend auf unserer Erfahrung hast du möglicherweise einen falschen Wert eingegeben. Bitte
                    ...
                    {kilometers && (
                      <div className="space-y-6">
                        <div className="space-y-2">
                          <label className="block text-lg font-medium text-gray-700">Wann planst du den Verkauf?</label>
                          <CustomSelect
                            value={planSell}
                        ...
                        {preferSell && (
                          <div className="space-y-2">
                            <label className="block text-lg font-medium text-gray-700">
                              Planst du, dein Auto für ein neues in Zahlung zu geben?
                            </label>
                        ...
                        {preferSell && (
                          <div className="flex items-center space-x-2 border-[1px] border-solid border-black px-2 py-4 rounded-md">
                            ...
                            <label className="text-sm text-gray-700">
                              Ich bin mit dem Erhalt meiner Bewertung per E-Mail sowie werblicher E-Mails zum Service
                              von WKDA GmbH einverstanden (keine Sorge, du kannst dich jederzeit wieder abmelden).
                            </label>
                          </div>
                        )}
                      </div>
